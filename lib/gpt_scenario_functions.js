...
export function getGroupFullPath(groupId) {
  let params = { headers: { "Accept": "application/json", "PRIVATE-TOKEN": `${__ENV.ACCESS_TOKEN}` } };
  let res = http.get(`${__ENV.ENVIRONMENT_URL}/api/v4/groups/${groupId}`, params);
  let groupFullPath = JSON.parse(res.body)['full_path'];
...
export function deleteGroup(groupId) {
  ...
  // Immediately delete a subgroup if it is marked for deletion.
  // Applicable to Premium and Ultimate tiers
  // if (envVersionIsHigherThan('16.0.0') && (__ENV.ENVIRONMENT_LICENSE === 'premium' || __ENV.ENVIRONMENT_LICENSE === 'ultimate')) {
    let groupFullPath = getGroupFullPath(groupId);
    let formdata = { permanently_remove: true, full_path: groupFullPath };
    console.log(`Removing group ${groupFullPath} permanently...`);
    res = http.del(`${__ENV.ENVIRONMENT_URL}/api/v4/groups/${groupId}`, formdata, params);
  // }
